...
public class SoundSystem : MonoBehaviour
{
    ...
    {
        PlayMusic(e.clip, e.volume, e.transitionTime);
    }

    public void PlayMusic(AudioClip clip, float volume = 0.5f, float transitionTime = 1.0f)
    {
        if (clip == null)
            return;
    ...
    void IsPlayingMusic(IsPlayingMusicEvent e)
    {
        e.isPlaying = IsPlayingMusic(e.clip);
    }
    ...
    public void StopMusic(float fade = 1.0f)
    {
        if (m_currentSource1)
        {
            m_musicSource1.DOFade(0, fade).OnComplete(() =>
            {
                if (this != null)
                    m_musicSource1.Stop();
            });
        ...
        {
            m_musicSource2.DOFade(0, fade).OnComplete(() =>
            {
                if (this != null)
                    m_musicSource2.Stop();
            });
    ...
    public void IsPlayingSound(IsPlayingSoundEvent e)
    {
        e.isPlaying = IsPlayingSound(e.clip);
    }
    ...
    void PlayLoop(PlaySoundLoopEvent e)
    {
        e.id = PlayLoop(e.clip, e.volume, e.fade);
    }
    ...
    void StopLoop(StopSoundLoopEvent e)
    {
        StopLoop(e.id, e.fade);
    }
